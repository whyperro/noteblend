...
export const archive = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const recursiveArchive = async (documenId: Id<"documents">) => {
...
export const removeIcon = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if(!identity){
      throw new Error("No autorizado sin identidad")
    }
    ...
    if(existingDocument.userId !== userId) {
      throw new Error(`No autorizado ID: ${userId} - ${existingDocument._id}`)
    }
